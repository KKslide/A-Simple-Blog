<template>
  ...
  >
    <el-steps style="width:60%;margin:0 auto 20px;" :active="formStepIndex" align-center finish-status="success">
      <el-step title="输入原始账号密码" />
      <el-step title="输入新账号密码" />
    </el-steps>
    <el-form ref="userFormRef" :model="userForm" :rules="userFormRules"
       @submit.native.prevent  @keyup.enter.native="saveHandler($event)"
    >
...
function saveHandler () {
  ...
  userFormRef.value.validate(valid => {
    if (valid) {
      const { id = 1 } = UserStore.$state // 后面再做持久化存储
